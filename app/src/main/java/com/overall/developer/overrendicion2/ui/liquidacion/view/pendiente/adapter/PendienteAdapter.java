package com.overall.developer.overrendicion2.ui.liquidacion.view.pendiente.adapter;
...
import com.overall.developer.overrendicion2.ui.liquidacion.view.datosGenerales.DatosGeneralesActivity;
...
/*        Animation animation = AnimationUtils.loadAnimation(mContext, (position > lastPosition) ? R.anim.card_view_fall_down: R.anim.card_view_fall_down);
       ...
       holder.btnEnviarResumen.setOnClickListener(v ->
                {
                    //21-03 por Gustavo M. para que envíe correo solo cuando tenga conexión a internet
                    if (Util.isOnline()) {
                        ((PendienteActivity) mContext).sendResumeEmail(liquidacionBean.getCodLiquidacion());
                    }else{
                        Toast.makeText(mContext,mContext.getResources().getString(R.string.errorInternet),Toast.LENGTH_SHORT).show();
                    }
                });
